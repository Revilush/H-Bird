...
public class QuickSynonymApplier {
	...
	static {
		// read synonyms and prepare the map
		List<List<String>> rawSyns = SynonymReader.getSynonymsRaw();
		
		String pattern;
		List<String> regex = new ArrayList<>();
		for (List<String> synList : rawSyns) {
			regex.clear();
			for (int i=0; i < synList.size(); i++) {
				regex.add(RefreshSynonyms.convertToSynRegex(synList.get(i)));
			}
			// sort the syn-list by length - DESC: longest on top
			regex.sort(Comparator.comparing(String::length).reversed());
			pattern = StringUtils.join(regex, "|");
			for (String kwd : synList) {
...
//		String[] grams = dataTexts[0].split(" ");
	...
	public static void main(String[] arg) {
		...
		System.out.println(textNGrams("shall have", 4));
	}
