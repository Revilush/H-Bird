...
public class DiffMatchPatch extends TextDiffHelper {
	 ...
	 */
	public float Diff_Timeout = 10.0f;
	/**
	...
	public DiffMatchPatch(boolean ignoreCase, boolean ignorePunctuations) {
	...
	public List<DiffDetail> getDiff(String text1, String text2) {
		List<Diff> diffs = diff_wordMode(text1, text2);
		List<DiffDetail> resp = convertNativeDiffList(diffs);
		return resp;
	}

	/**
	 * Pass-in a List<Diff> - the list that 'diff_wordMode(...)' returns, and this method will convert that list to standard 'List<DiffDetail>'.
	 */
	@SuppressWarnings("unchecked")
	@Override
	public List<DiffDetail> convertNativeDiffList(List<? extends Object> diffList) {
		List<Diff> diffs = (List<Diff>) diffList;
		List<DiffDetail> resp = new ArrayList<>();
		for (Diff dif : diffs) {
			resp.add(new DiffDetail(dif.operation.name(), dif.text));
		}
		return resp;
	}
	...
	/**
	 * Added by Praveen: (based on https://github.com/google/diff-match-patch/wiki/Line-or-Word-Diffs).
	 * @param text1	sentence#1
	 * @param text2	sentence#2
	 * @return diffs (inserts/deletion/equals) i.e. 	[{"operation":"EQUAL","text":"004 015 "},{"operation":"DELETE","text":"016 "},{"operation":"INSERT","text":"017 020 "},{"operation":"EQUAL","text":"003 000 005 006 "},{"operation":"DELETE","text":"010 "},{"operation":"INSERT","text":"011 "},{"operation":"EQUAL","text":"001 008 009 014 000 007 "},{"operation":"INSERT","text":"015 "},{"operation":"EQUAL","text":"002 000 001 013 012 002 011"}]
	 */
	public LinkedList<Diff> diff_wordMode(String text1, String text2) {
		LinesToCharsResult a = diff_linesToWords(text1, text2);
		text1 = a.chars1;
		text2 = a.chars2;
		List<String> linearray = a.lineArray;

		LinkedList<Diff> diffs = diff_main(text1, text2, false, Long.MAX_VALUE);
		diff_wordsToLines(diffs, linearray);
		return diffs;
	}

	/**
	 * Counts words that are in either one of the sentences (inserted/deleted) - compared sequentially. Ignores the ones that are equal (in both sentences).
	 * @param text1
	 * @param text2
	 * @return
	 */
	public Integer countDiffWords(String text1, String text2) {
		LinkedList<Diff> diffs = diff_wordMode(text1, text2);
		if (null == diffs  ||  diffs.size() == 0)
			return null;
		int difCount = 0;
		for (Diff d : diffs) {
			if (d.operation == Operation.EQUAL)
				continue;
			difCount += d.text.trim().split(" ").length;		// its either insert or delete. count each word
		}
		return difCount;
	}
	 ...
	 *                   first to identify the changed areas. If true, then run a
	...
	private LinkedList<Diff> diff_compute(String text1, String text2, boolean checklines, long deadline) {
		...
		if (checklines && text1.length() >= 1  &&  text2.length() >= 1) {
	...
	protected LinesToCharsResult diff_linesToWords(String text1, String text2) {
		...
		// Allocate 2/3rds of the space for text1, the rest for text2.
		String chars1 = diff_wordsToCharsMunge(text1, lineArray, lineHash, 40000);
		String chars2 = diff_wordsToCharsMunge(text2, lineArray, lineHash, 65535);
		return new LinesToCharsResult(chars1, chars2, lineArray);
	}
	...
	private String diff_wordsToCharsMunge(String text, List<String> lineArray, Map<String, Integer> lineHash,
		...
		while (lineEnd < text.length() - 1) {
			...
			if (ignoreCase)
				noPunctLine = noPunctLine.toLowerCase();
			if (ignorePunctuations) {
 				noPunctLine = getWordWithoutPunctuation(noPunctLine.trim());
			}
			
			if (lineHash.containsKey(noPunctLine)) {
				chars.append(String.valueOf((char) (int) lineHash.get(noPunctLine)));
			} else {
				if (lineArray.size() == maxLines) {
					// Bail out at 65535 because
					// String.valueOf((char) 65536).equals(String.valueOf(((char) 0)))
					line = text.substring(lineStart);
					noPunctLine = line.trim();					// there may be multiple words in this last possible snippet - no point in taking only first non-punct word.
					lineEnd = text.length();
				}
				lineArray.add(line);
				lineHash.put(noPunctLine, lineArray.size() - 1);			//
				chars.append(String.valueOf((char) (lineArray.size() - 1)));
			}
			lineStart = lineEnd + 1;
		}
	...
	private String getWordWithoutPunctuation(String word) {
		String w = new String(word);
		// get only the word, excluding any puncts (either at start/ends etc)
		Pattern p = Pattern.compile("(?<=[\\p{Punct}}])([^\\p{Punct}}]+)|([^\\p{Punct}}]+)(?=[\\p{Punct}}])");
	 ...
	 * @param lineArray List of unique strings.
	 */
	protected void diff_wordsToLines(List<Diff> diffs, List<String> lineArray) {
		StringBuilder text;
		for (Diff diff : diffs) {
			text = new StringBuilder();
			for (int j = 0; j < diff.text.length(); j++) {
	...
	public static class Patch {
		...
		/**
		 * Emulate GNU diff's format. Header: @@ -382,8 +481,9 @@ Indices are printed as
		 * 1-based, not 0-based.
		 * 
	...
	public static void main(String[] arg) throws JsonProcessingException {
		DiffMatchPatch df = new DiffMatchPatch(true, true);
		long start = System.currentTimeMillis();
		
		String t1 = "004 015 016 003 000 005 006 010 001 008 009 014 000 007 002 000 001 013 012 002 011";
		String t2 = "004 015 017 020 003 000 005 006 011 001 008 009 014 000 007 015 002 000 001 013 012 002 011";

		t1 = "Notwithstanding any other provision of this Indenture, the right of any Holder to receive payment of principal of and interest on the Notes held by such Holder, on or after the respective due dates expressed or provided for in the Notes, or to bring suit for the enforcement of any such payment on or after such respective dates, shall not be impaired or affected without the consent of such Holder.";
		t2 = "Rights of Holders To Receive Payment. Notwithstanding any other provision of this Indenture (including, without limitation, Section 6.6), the right of any Holder to receive payment of principal of, premium, if any, or interest, including Additional Interest, if any, on the Notes held by such Holder, on or after the respective due dates expressed or provided for in the Notes, or to bring suit for the enforcement of any such payment on or after such respective dates, shall not be impaired or affected without the consent of such Holder.";
